...
class FCM:
    # Class taken from CausalAssembly repository.
    ...
    # FIXME: We need to allow to also build "FCMs" from source nodes only
    def input_sources(self, sources: list[Eq]):
        """
        Automatically builds up DAG according to the source
        node definitions fed in.
        Args:
            sources (list): list of sympy equations generated as:
                    ```[python]
                    x, y = symbols('x, y')
                    distr_x = Eq(x, Normal('x', 0,1))
                    distr_y = Eq(y, Normal('y', 10,2))
                    sources = [distr_x, distr_y]
                    ```
        """
        ...
        term_dict = {}
        for term in sources:
            if len(term.lhs.free_symbols) == 1:
                if len(term.rhs.free_symbols) == 1:
                    if all(
                        [
                            isinstance(term.rhs, RandomSymbol),
                            str(list(term.lhs.free_symbols)[0])
                            == str(list(term.rhs.free_symbols)[0]),
                        ]
                    ):
                        nodes_implied.append(list(term.lhs.free_symbols)[0])
                        term_dict[term.lhs] = {"term": term.rhs}
                elif len(term.rhs.free_symbols) == 0:
                    # e.g. because a constant value such as 0 has been set
                    nodes_implied.append(list(term.lhs.free_symbols)[0])
                    term_dict[term.lhs] = {"term": term.rhs}
    ...
    def interventional_amat(self, which_intervention: int | str) -> pd.DataFrame:
        ...
        """
        if (
            isinstance(which_intervention, str)
            and which_intervention not in self.interventions
        ):
            raise ValueError("The intervention you provide does not exist.")

        if isinstance(which_intervention, int) and which_intervention > len(
            self.interventions
        ):
    ...
    def causal_order_of(self, which_graph: nx.DiGraph) -> list[Symbol]:
        ...
        ensure uniqueness, we additionally sort lexicographically.

        Returns:
            list[Symbol]: Causal order
        """
        return list(
            nx.lexicographical_topological_sort(which_graph, key=lambda x: str(x))
        )
    ...
    def source_nodes_of(self, which_graph: nx.DiGraph) -> list:
        ...
        Returns:
            list: List of nodes.
        """
        return [node for node, degree in which_graph.in_degree() if degree == 0]
        ...
        for term in fcm:
            if not isinstance(term.rhs, RandomSymbol):
                if term.rhs.atoms(RandomSymbol):
                    ...
                    edges_implied.extend(
                        [(atom, term.lhs) for atom in term.rhs.atoms(Symbol)]
                    )
    ...
    def interventional_sample(
        ...
        you'll have switched to another intervention for sampling from the corresponding
    ...
    def _sample(
        ...
        source_nodes = self.source_nodes_of(which_graph=which_graph)
        causal_order = self.causal_order_of(which_graph=which_graph)

        # Debug: Check if there are any nodes without a term
        miss_term = []
        for node in causal_order:
            if "term" not in which_graph.nodes[node]:
                miss_term.append(node)
        if miss_term:
            raise ValueError(
                f"Nodes {miss_term} have no term assigned. Please assign a term to all nodes."
            )
        
        # Prepare a dict for faster lookup of FCM expressions
        nodes_dict = dict(which_graph.nodes)
        
        for node in causal_order:
            # If node is a source node, sample from the source distribution.
            if node in source_nodes:
                if isinstance(nodes_dict[node]["term"], RandomSymbol):
                    if not self.__distribution_parameters_explicit(node, which_graph=which_graph):
                        raise AttributeError("Source node parameters need to be given explicitly.")
                    df[node.name] = sympy_sample(
                        nodes_dict[node]["term"],
                        seed=self._random_state,
                        size=size,
                    )

                elif isinstance(nodes_dict[node]["term"], Number):
                    df[node.name] = np.repeat(nodes_dict[node]["term"], repeats=size)
            ...
            # If node is not a source node, we need to evaluate the expression
            else:
                # Using a dict, the lookup time is O(1)
                fcm_expr = nodes_dict[node]["term"]
                fcm_expr_random_symbols = fcm_expr.atoms(RandomSymbol)
                explicit_pars = self.__distribution_parameters_explicit(node, which_graph=which_graph)

                if fcm_expr_random_symbols:
                    if explicit_pars:
                        val = fcm_expr_random_symbols.pop()
                        df[str(val)] = sympy_sample(
                            val,
                            size=size,
                            seed=self._random_state,
                        )
                    else:
                        df[str(fcm_expr_random_symbols.pop())] = np.zeros(size)

                df[node.name] = self.__eval_expression(df=df, fcm_expr=fcm_expr)

                if fcm_expr_random_symbols and not explicit_pars:
                    ...
                    noise_var = df[node.name].var() / snr
                    df[node.name] = df[node.name] + sympy_sample(
                        fcm_expr_random_symbols
                        .pop()
                ...
                if additive_gaussian_noise:
                    if fcm_expr.atoms(RandomSymbol):
                        ...
                        noise = symbols("noise")
                        noise_var = df[node.name].var() / snr
                        df["noise"] = self._random_state.normal(
                            loc=0, scale=np.sqrt(noise_var), size=size
                        )
                        fcm_expr = nodes_dict[node]["term"] + noise
                        df[node.name] = self.__eval_expression(df=df, fcm_expr=fcm_expr)

        self.last_df = df[[node.name for node in self.causal_order]]

        return self.last_df

    def __unfree_symbol(self, fcm_expr) -> set[Symbol]:
        random_symbols = fcm_expr.atoms(Symbol).difference(fcm_expr.free_symbols)
        return {
            unfree
            for unfree in random_symbols
    ...
    def __distribution_parameters_explicit(
        self, node: Symbol, which_graph: nx.DiGraph
    ) -> bool:
        ...
        Returns:
            bool:
        """
        return len(which_graph.nodes[node]["term"].free_symbols) == len(
            which_graph.nodes[node]["term"].atoms(Symbol)
        )
    ...
    def __source_df_condition(self, source_df: pd.DataFrame) -> bool:
        ...
        return {str(col) for col in source_df.columns}.issubset(
            {node.name for node in self.source_nodes}
        )

    def intervene_on(self, intervened_set: dict[Symbol, RandomSymbol | float]):
        ...
        Args:
            nodes_values (dict[Symbol, RandomSymbol | float]): either single real
                number or RandomSymbol. If you provide more than one
        ...
        Example:
            ...
            ```
        """
        # Convert intervened set to sympy
        if isinstance(intervened_set, dict) and all(isinstance(key, str) for key in intervened_set.keys()):
            intervened_set = {
            symbols(key): value for key, value in intervened_set.items()
            }

        if not set(intervened_set).issubset(set(self.nodes)):
        ...
        for node, value in intervened_set.items():
            if isinstance(value, str):
                value = eval(value)  
        
            intervention = Eq(node, value)
            old_incoming = self.graph.in_edges(node)
        ...
        self.mutilated_dags[f"do({list(intervened_set.keys())})"] = (
            mutilated_dag  # specifying the same set twice will override
        )

    def show(self, header: str | None = None, with_nodenames: bool = True) -> None:
        ...
        if header is None:
            header = ""
        return self._show(
            which_graph=self.graph, header=header, with_nodenames=with_nodenames
        )
    ...
    def _show(self, which_graph: nx.DiGraph, header: str, with_nodenames: bool):
        ...
        nx.draw_networkx_nodes(
            ...
            node_color=[
                ...
                for _, d in which_graph.in_degree(self.nodes)
            ],
            node_size=[
                500 * (d + 1) / (max_out_degree + 1)
                for _, d in which_graph.out_degree(self.nodes)
            ],
        )
    ...
    def causal_order(self) -> list[Symbol]:
        ...
        """
        return list(
            nx.lexicographical_topological_sort(self.graph, key=lambda x: str(x))
        )
